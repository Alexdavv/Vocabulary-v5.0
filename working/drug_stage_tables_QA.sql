...
FROM (
	...
	SELECT a.concept_code,
		'concept_id_2 exists but doesnt belong to any concept'
	FROM relationship_to_concept r
	JOIN drug_concept_stage a ON a.concept_code = r.concept_code_1
	...
	SELECT DISTINCT drug_concept_code,
		'unit doesnt exist in concept_table'
	FROM ds_stage
	WHERE (
			amount_unit NOT IN (
				SELECT concept_code
	...
	SELECT DISTINCT s.drug_concept_code,
		'ds_stage has drug_codes absent in drug_concept_stage'
	FROM ds_stage s
	...
	SELECT DISTINCT s.drug_concept_code,
		'impossible combination of values and units in ds_stage'
	FROM ds_stage s
	...
	SELECT drug_concept_code,
		'homeopathy in amount, need to check'
	FROM concept c
	...
	JOIN relationship_to_concept rtc ON amount_unit = rtc.concept_code_1
	...
	SELECT drug_concept_code,
		'numerator should be placed into amount'
	FROM concept c
	JOIN relationship_to_concept rc2 ON concept_id_2 = concept_id
	JOIN internal_relationship_stage irs ON rc2.concept_code_1 = irs.concept_code_2
	JOIN ds_stage ds ON ds.drug_concept_code = irs.concept_code_1
	JOIN relationship_to_concept rtc ON numerator_unit = rtc.concept_code_1
		AND rtc.concept_id_2 IN (
			9324,
			9325
			)
	...
	SELECT concept_code_1,
		'internal_relationship_dublicates'
	FROM (
		SELECT concept_code_1,
			concept_code_2
	...
	SELECT DISTINCT a.drug_concept_code,
		'concept overlaps with other one by target concept, please look also onto rigth sight of query result'
	FROM (
		SELECT DISTINCT a.amount_unit,
			a.numerator_unit,
			cs.concept_code,
			cs.concept_name AS old_name,
		...
		JOIN (
			...
			FROM ds_stage a
			JOIN relationship_to_concept b ON a.ingredient_concept_code = b.concept_code_1
			GROUP BY a.drug_concept_code,
				b.concept_id_2
			HAVING COUNT(*) > 1
			) c ON c.DRUG_CONCEPT_CODE = a.DRUG_CONCEPT_CODE
			AND c.CONCEPT_ID_2 = b.CONCEPT_ID_2
		WHERE precedence = 1
		) a
	JOIN (
		SELECT DISTINCT a.amount_unit,
		...
		JOIN (
			...
			AND c.CONCEPT_ID_2 = b.CONCEPT_ID_2
		WHERE precedence = 1
		) b ON a.RxName = b.RxName
		AND a.drug_concept_code = b.drug_concept_code
		AND (
			a.AMOUNT_UNIT != b.amount_unit
			OR a.NUMERATOR_UNIT != b.NUMERATOR_UNIT
			OR a.NUMERATOR_UNIT IS NULL
			AND b.NUMERATOR_UNIT IS NOT NULL
			OR a.AMOUNT_UNIT IS NULL
	...
	SELECT concept_code,
		'non-standard ingredients dont have replacemt mapping '
	FROM drug_concept_stage
	...
	SELECT drug_concept_code,
		'drug_ingr relationship is missing from irs'
	FROM ds_Stage
	WHERE (
			drug_concept_code,
			ingredient_concept_code
